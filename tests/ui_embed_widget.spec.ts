...
test('UI single panel loads in embed widget', async ({ page }) => {
  ...
  await expect(page.locator(SELECTORS.DH_REACT_PANEL_VISIBLE)).toBeVisible();
...
test('UI multi panel loads in embed widget', async ({ page }) => {
  ...
  await expect(page.locator(SELECTORS.DH_REACT_PANEL)).toHaveCount(2);
...
test('UI dashboard loads in embed widget', async ({ page }) => {
  ...
  await expect(page.locator(SELECTORS.DH_REACT_PANEL)).toHaveCount(4);
  await waitForLoad(page);
