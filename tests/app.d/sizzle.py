...
for _idx, _sym in enumerate(SYMBOLS):
    ...
    _vol = 1.2 + _idx * 0.25
    _t = (
        empty_table(_SEED_ROWS_PER_SYM)
        .update(
            [
                f"Sym = `{_sym}`",
...
_tick_base = time_table("PT00:00:00.500000000").drop_columns("Timestamp")
tick_table = (
    _tick_base.update(
        [
            "SymIdx = (int)(ii % 5)",
            "Sym    = new String[]{`AAPL`, `MSFT`, `GOOG`, `AMZN`, `TSLA`}[SymIdx]",
            f"Timestamp = _seed_start + (long)(({_SEED_ROWS_PER_SYM}L + ii / 5L) * 60_000_000_000L)",
