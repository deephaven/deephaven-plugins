...
- For Python changes, ensure the public API surface is consistent with the rest of the plugin and that internal details are not unnecessarily exposed. When a public Python API is added or changed, flag if there is no corresponding documentation update in `plugins/<plugin>/docs/`.
