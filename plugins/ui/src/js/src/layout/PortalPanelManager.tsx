...
import {
  type PortalClosedPayload,
  type PortalOpenedPayload,
...
function PortalPanelManager({
  ...
  const handlePortalOpened = useCallback(
    ({ container, element }: PortalOpenedPayload) => {
      setPortals(prevPortals => {
        ...
        const panelId = Array.isArray(containerId)
          ? containerId[0]
          : containerId;
