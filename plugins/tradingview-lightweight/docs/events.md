...
| `hoveredSeriesId` | `str`                                    | The generated id of the hovered series: `series_<n>`, or `series_<n>_<key>` for a `by=` partition. It never changes with the title, so it is a safe key for server-side lookup even when two titles collide. Omitted when no series is hovered.                       |
