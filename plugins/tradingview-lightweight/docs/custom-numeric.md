...
> [!NOTE]
> Viewport-aware downsampling currently only applies to time-axis charts (`chart_type == "standard"`). Custom-numeric charts ship raw rows to the client — keep the input table modest (tens of thousands of rows) or pre-aggregate server-side.
