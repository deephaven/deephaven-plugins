...
A standalone per-type call (e.g. `tvl.line(table, ...)`) is just shorthand for `tvl.chart(tvl.line(table, ...))` with default chart options. The moment you need _any_ chart-level option, wrap the series call in `tvl.chart(..., option=value)`.
