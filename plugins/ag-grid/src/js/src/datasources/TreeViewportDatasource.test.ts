...
describe('TreeViewportDatasource', () => {
  let mockDh: typeof DhType;
  let mockTable: DhType.TreeTable;
  let mockGridApi: GridApi;
  let datasource: TreeViewportDatasource;
  ...
  beforeEach(() => {
    ...
    mockTable = TestUtils.createMockProxy<DhType.TreeTable>({
      size: 100,
    });
    ...
    datasource = new TreeViewportDatasource(mockDh, mockTable);
