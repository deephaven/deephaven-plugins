...
`{{ cookiecutter.__component_name }}.py` defines a simple Python component that maps to a React element, as well as `{{ cookiecutter.__event_sender_name }}`, a function that sends an event to the client using `use_send_event`. These can be modified to have other plugin functionality or replaced with different objects entirely, depending on the plugin's needs.  
...
This template also demonstrates sending events from the server to the client using `use_send_event`.
The `{{ cookiecutter.__event_sender_name }}` function sends an event to the client-side handler registered in `{{ cookiecutter.__js_plugin_obj }}.ts`,
and the example handler displays the message in an alert. Like `ui.toast`, it must be called from the render thread
of a `@ui.component`:

```python
from deephaven import ui
from {{ cookiecutter.python_project_name }} import {{ cookiecutter.__event_sender_name }}


@ui.component
def send_event_on_click():
    render_queue = ui.use_render_queue()

    def handle_press():
        # `{{ cookiecutter.__event_sender_name }}` must run on the render thread, so queue it from the press handler.
        render_queue(lambda: {{ cookiecutter.__event_sender_name }}("Hello from the server!"))

    return ui.button("Send event", on_press=handle_press)


example = send_event_on_click()
```
