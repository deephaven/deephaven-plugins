...
# Set the log level for the deephaven.plugin logger to DEBUG
logging.getLogger("deephaven.plugin").setLevel(level=logging.DEBUG)
```
...
```python
# Only log warnings from deephaven.plugin.hooks
logging.getLogger("deephaven.plugin.hooks").setLevel(level=logging.WARNING)

# Log all debug messages from the render module specifically
logging.getLogger("deephaven.plugin.render").setLevel(level=logging.DEBUG)
```
