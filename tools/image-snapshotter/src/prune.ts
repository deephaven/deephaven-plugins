...
interface ObjEntry {
  type?: unknown;
  data?: { image?: unknown } & Record<string, unknown>;
}

export function collectReferencedAssets(snapshotsDir: string): Set<string> {
  const referenced = new Set<string>();
  if (!existsSync(snapshotsDir)) return referenced;
...
export function pruneOrphanAssets(snapshotsDir: string): PruneResult {
  const assetsDir = join(snapshotsDir, 'assets');
  const result: PruneResult = { deleted: [], kept: 0, referenced: 0 };
