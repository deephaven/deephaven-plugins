...
function main(): number {
  ...
  if (!docsDir) {
    ...
    if (root) docsDir = join(root, 'docs');
  }
  if (!docsDir) {
    process.stderr.write(
      'image-snapshotter-fixture-gen: --docs is required (or set SNAPSHOTTER_PLUGIN_ROOT)\n',
    );
    return 2;
  }
  if (!args.out) {
  ...
  for (const [md5, prefix] of result.prefixMap) prefixMap[md5] = prefix;
  const prefixMapPath = join(outAbs, 'prefix-map.json');
  writeFileSync(prefixMapPath, JSON.stringify(prefixMap, null, 2) + '\n', 'utf8');

  process.stdout.write(
    `image-snapshotter-fixture-gen: wrote ${result.blockCount} block file(s) + manifest to ${outAbs}\n`,
  );
  return 0;
}
