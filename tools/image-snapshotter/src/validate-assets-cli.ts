...
interface Envelope {
  file?: string;
  objects?: Record<string, { type?: string; data?: { image?: string } }>;
}
...
export function validateSnapshotAssets(snapshotsDir: string): ValidateResult {
  ...
  for (const name of entries) {
    ...
    for (const [objectName, entry] of Object.entries(objects)) {
      ...
      if (typeof imageRel !== 'string') continue; // not a Pass-2 entry
      imageRefsScanned += 1;
...
function pluginRoot(): string {
  ...
  process.stderr.write(
    'image-snapshotter-validate-assets: SNAPSHOTTER_PLUGIN_ROOT or a snapshots dir argument is required\n',
  );
  process.exit(2);
}
...
function main(): number {
  ...
  const { missing, envelopesScanned, imageRefsScanned } =
    validateSnapshotAssets(snapshotsDir);

  process.stdout.write(
    `validated ${envelopesScanned} envelope(s); ` +
  ...
  for (const m of missing) {
    process.stderr.write(
      `  ${m.envelopePath} :: ${m.objectName} -> ${m.imageRel} (not on disk)\n`,
    );
  }
  process.stderr.write(
