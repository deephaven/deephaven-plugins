...
function parseArgs(argv) {
    const a = { update: false, pruneOnly: false, check: false, help: false };
    for (const v of argv) {
        switch (v) {
            case '--update':
                a.update = true;
...
function pluginRoot() {
    ...
    process.stderr.write('image-snapshotter: SNAPSHOTTER_PLUGIN_ROOT is required\n');
    process.exit(2);
}
function runPlaywright(toolDir) {
...
function checkClean(snapshotsDir) {
...
function main() {
    ...
    process.stdout.write(`pruned ${pruneResult.deleted.length} orphan asset(s); ` +
        `kept ${pruneResult.kept}, referenced ${pruneResult.referenced}\n`);
    for (const p of pruneResult.deleted)
