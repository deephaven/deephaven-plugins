...
[ "$dev" = true ] && extra=".dev0"
case "$package" in
        ag-grid | matplotlib | pivot | plotly | plotly-express | python-remote-file-source | theme-pack | tradingview-lightweight | ui | utilities | packaging)
...
if [ "$dev" != true ]; then
    case "$package" in
        ag-grid | auth-keycloak | matplotlib | pivot | pivot-builder | plotly | plotly-express | python-remote-file-source | table-example |  theme-pack | tradingview-lightweight | ui)
